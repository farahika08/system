	...
	public function listUser()
	{
		...
		while ($user = mysqli_fetch_assoc($result)) {
			...
			if ($user['user_type'] == 'admin') {
				...
				$userRole = '<span class="label label-warning">Technician</span>';
			}
	...
	public function insert()
	{
		if ($this->userName && $this->email) {
			...
			if (!mysqli_query($this->dbConnect, $queryInsert)) {
				echo "Error inserting user: " . mysqli_error($this->dbConnect);
			}

			// If the user is a technician, insert the specialization data into technician_table
			if ($this->role == 'technician' && !empty($this->specialization)) {
				// Get the last inserted user ID
				$lastInsertId = mysqli_insert_id($this->dbConnect);

				// Insert specialization into technician table
				$queryInsertSpecialization = "
                INSERT INTO hd_technicians (user_id, specialization) VALUES ('" . $lastInsertId . "', '" . $this->specialization . "')";
				if (!mysqli_query($this->dbConnect, $queryInsertSpecialization)) {
					echo "Error inserting technician specialization: " . mysqli_error($this->dbConnect);
				}
			}
		}
	}
